...
if(data.token)  
  ...
  let validationSchema = Yup.object({
    ...
    newPassword: Yup.string().required().matches(/^[0-9a-z]{3,9}$/, 'password is not valied'),
  })
    ...
    <div className='container p-2'>
      <div className='m-auto w-3/4'>
        ...
        <form className='my-3' onSubmit={formik.handleSubmit}>
          ...
          <div className="relative my-5">
            <input type="password" id="newPassword" name='newPassword' className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-2 border-gray-900 appearance-none dark:text-white dark:border-gray-900 dark:focus:border-green-900 focus:outline-none focus:ring-0 focus:border-green-600 peer" placeholder=" " onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.newPassword} />
            <label htmlFor="newPassword" className="absolute text-sm text-gray-900 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white dark:bg-gray-900 px-2 peer-focus:px-2 peer-focus:text-green-600 peer-focus:dark:text-green-900 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">newPassword :</label>

          </div>
          {formik.errors.newPassword && formik.touched.newPassword && (<span className='text-red-500'>{formik.errors.newPassword}</span>)}
